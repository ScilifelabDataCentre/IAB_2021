...
trend_data_fell = pd.read_excel(
    "Data/SciLifeLab_cf_subj_cat_fellows.xlsx",
    sheet_name="SciLifeLab_cf_subj_cat_fellows",
    header=0,
    engine="openpyxl",
...
trend_data_fell_sub = trend_data_fell_sub[
    (trend_data_fell_sub["Doc_type_code_rev"] == "RV")
    | (trend_data_fell_sub["Doc_type_code_rev"] == "AR")
    | (trend_data_fell_sub["Doc_type_code_rev"] == "PP")
]
trend_group = (
    trend_data_fell_sub.groupby(["Subject_category"])
...
total_papers = len(trend_data_fell_sub.drop_duplicates(subset="UT", keep="first"))
