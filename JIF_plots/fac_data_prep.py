...
Pubs_JIF_raw = pd.read_excel(
    "DataforJIF/Facilities_20210419.xlsx",
    sheet_name="Sheet1",
    header=0,
    engine="openpyxl",
...
JIF_sub_group_fac = JIF_sub.groupby(["Year", "JIFcat"]).size().reset_index()

JIF_sub_group_fac.columns = ["Year", "JIFcat", "Count"]


# Use this to check that the sums are as expected given the original publication files
# JIF_sub_group_fac.to_excel("facility_JIF_groups.xlsx")
