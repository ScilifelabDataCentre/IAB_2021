"""This script generates all the wordclouds involving keyword plus for IAB 2021 report"""
...
def makekeywordpluswordcloud(data, Thresh_num, group_name, stopwords):
    wordcloud = WordCloud(
    ...
    if not os.path.isdir("Plots/keywordplusclouds"):
        os.mkdir("Plots/keywordplusclouds")
    # savefig will save the figure (at resolution 300dpi - good enoough for print)
    plt.savefig(
        "Plots/keywordplusclouds/{}_keywordpluswordcloud.png".format(group_name),
        dpi=300,
    )
...
makekeywordpluswordcloud(
    keyplus_words_fell_range, 10, "fell_keyplus_2years", stopwords_fell
)
makekeywordpluswordcloud(
    keyplus_words_fell_year1, 10, "fell_keyplus_{}".format(year1), stopwords_fell
)
makekeywordpluswordcloud(
    keyplus_words_fell_year2, 10, "fell_keyplus_{}".format(year2), stopwords_fell
)
makekeywordpluswordcloud(
    keyplus_words_fac_range, 10, "fac_keyplus_2years", stopwords_fac
)
makekeywordpluswordcloud(
    keyplus_words_fac_year1, 10, "fac_keyplus_{}".format(year1), stopwords_fac
)
makekeywordpluswordcloud(
    keyplus_words_fac_year2, 10, "fac_keyplus_{}".format(year2), stopwords_fac
)
makekeywordpluswordcloud(
    keyplus_words_aff_range, 10, "aff_keyplus_2years", stopwords_aff
)
makekeywordpluswordcloud(
    keyplus_words_aff_year1, 10, "aff_keyplus_{}".format(year1), stopwords_aff
)
makekeywordpluswordcloud(
    keyplus_words_aff_year2, 10, "aff_keyplus_{}".format(year2), stopwords_aff
)
