...
using this script. Note: require at least 6 categories
in the matrix. Only include facilities that collaborated
with at least one other facility, or you'll see error
...
df = pd.read_excel('/Users/liahu895/Documents/testdata/covid19portal.xlsx',
    sheet_name='Publications',
    engine='openpyxl')
#extract funders labels from the excel
funders = df['Labels'].str.split('|')
funders=funders.tolist()
#list of labels that should be considered 
labels=['Funder: VR', 'Funder: VR: Special COVID-19 funding',
    'Funder: Vinnova', #'Funder: KAW/SciLifeLab',
    'Funder: NordForsk', 'Funder: H2020'
    ]
#produce data matrix
G = nx.MultiGraph()
G = nx.from_edgelist((c for n_nodes in funders for c in combinations(n_nodes, r=2)),
...
#think w1800 and h1500 will be good  
fig.update_layout(autosize=False, width=1000, height=780)
...
for i, m in enumerate(labels):
    fig.add_annotation(dict(font=dict(color='black',size=12),
                                        ...
                                        showarrow=True,
                                        text=labels[i],
                                        xanchor='left' if ideograms[i].x[int((len(ideograms[i].x)/2))]>0 else 'right',
                                        yanchor='bottom' if ideograms[i].y[int((len(ideograms[i].y)/2))]>0 else 'top',
